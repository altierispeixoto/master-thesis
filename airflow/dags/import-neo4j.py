...
stopevents_processing = []
tracking_data_processing = []
event_stop_edges_processing = []
...
for i in range(delta.days + 1):
    ...
    stopevents_processing.append(
        DockerOperator(
            task_id='spark_etl_stop_events-{}'.format(datareferencia),
            image='altr/spark',
            api_version='auto',
            auto_remove=True,
            environment={
    ...
    tracking_data = '/spark/bin/spark-submit --master local[*] --executor-memory 10g --driver-memory 18g --conf spark.network.timeout=600s --driver-class-path ' \
                        '/spark-urbs-processing/jars/postgresql-42.2.8.jar  /spark-urbs-processing/tracking-data.py -d {}'.format(datareferencia)

    tracking_data_processing.append(
        DockerOperator(
            task_id='spark_etl_tracking_data-{}'.format(datareferencia),
                image='altr/spark',
                api_version='auto',
                auto_remove=True,#mem_limit='10g',
                environment={
                    'PYSPARK_PYTHON': "python3",
                    'SPARK_HOME': "/spark"
                },
    ...
    event_stop_edges = '/spark/bin/spark-submit --master local[*] --driver-class-path ' \
                   '/spark-urbs-processing/jars/postgresql-42.2.8.jar  /spark-urbs-processing/event-stop-edges.py -d {}'.format(datareferencia)

    event_stop_edges_processing.append(
        DockerOperator(
            task_id='spark_etl_event_stop_edges-{}'.format(datareferencia),
            image='altr/spark',
            api_version='auto',
            auto_remove=True,
            environment={
...
#
#
for task in range(0,len(stopevents_processing)):
    start >> stopevents_processing[task] >> tracking_data_processing[task] >> event_stop_edges_processing[task]
